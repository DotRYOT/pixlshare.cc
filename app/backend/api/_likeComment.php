...
require "../scripts/account/_notificationService.php"; // ← Added
...
// 🔍 Fetch COMMENT owner (not post owner!)
$sql = "SELECT UUID FROM comments WHERE CUID = ?";
$stmt = mysqli_prepare($conn_main, $sql);
mysqli_stmt_bind_param($stmt, "s", $PUID);
mysqli_stmt_execute($stmt);
mysqli_stmt_bind_result($stmt, $OwnerUUID);
...
if ($action === 'like') {
  ...
  if (!$success) {
    echo json_encode(['error' => 'Error liking comment.']);
    exit;
  }
  ...
  if (!$success) {
    echo json_encode(['error' => 'Error unliking comment.']);
    exit;
  }
