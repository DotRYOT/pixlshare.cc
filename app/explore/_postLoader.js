...
function handleScroll() {
  ...
  if (scrollTop + windowHeight >= docHeight - 300 && hasMore) {
    loadPosts();
  }
}
...
function loadPosts() {
  ...
  const url = `../backend/scripts/_exploreFeedLoader.php?filter=${currentFilter}&limit=${limit}&offset=${offset}`;
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      hasMore = data.posts.length === limit; // Stop loading if fewer than `limit` posts are returned
      spinner.classList.add("hidden");
      loading = false;
    })
    .catch((err) => {
    ...
    >
      <div class="topCardContainer">
        ...
        <div class="buttonTray">
          ...
          <button
            onclick="togglePostLike('${post.PUID}', this)"
            class="likeButton"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              id="like-icon-outline-${post.PUID}"
              style="display: ${unlikedStyle};"
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="liked"
              id="like-icon-full-${post.PUID}"
              style="display: ${likedStyle}; fill: red;"
              viewBox="0 -960 960 960"
            >
              <path
            ...
            <span id="like-count-${post.PUID}" class="likeCount"
              >${likeCount}</span
            >
          </button>
        </div>
