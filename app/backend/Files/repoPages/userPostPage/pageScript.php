...
// Spot where it will redirect non admin/poster to the suspended post page
...
$media_id = $dataArray['media_id'] ?? $dataArray['image_id'] ?? '';

$extension = pathinfo($media_id, PATHINFO_EXTENSION);
...
if (isset($_GET['action'])) {
  if ($_GET['action'] == 'like') {
    $sessionUUID = $_SESSION['user']['UUID'];
    $Path = "../../../../..";
    RemoveLikeNotification($sessionUUID, $PUID, $conn_main, $Path);
  }
}